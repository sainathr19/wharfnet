...
test tokens, canonical infra contracts, and a block explorer. This page covers
everything EVM-specific; see the [root README](../README.md) for install,
quickstart, and configuration basics.
...
forking with chain control: `wharfnet evm impersonate` lets you send transactions
as any address (a whale, a protocol admin) with no key.
...
no indexer or database — it's a static frontend talking straight to the chain.
Each EVM chain gets its own Otterscan on a dedicated port:
