//! A minimal JSON-RPC client for a running `surfpool` chain's RPC endpoint.
//! surfpool serves the standard Solana JSON-RPC — plus its `surfnet_*` cheatcodes
//! (`surfnet_timeTravel`, `surfnet_pauseClock`, …) — at the root path, so
//! [control](super::control) drives them through here. A dependency-free raw
//! socket keeps this in step with the orchestrator's readiness probe rather than
//! pulling in an HTTP client (mirroring the Starknet [`devnet`] client).
//!
//! [`devnet`]: crate::starknet::devnet
...
pub(crate) fn post(chain: &ChainEntry, body: &str) -> Result<String> {
    ...
    let mut stream = TcpStream::connect(format!("{host}:{port}")).with_context(|| {
        format!("connecting to surfpool at {host}:{port} — is the localnet up?")
    })?;
    ...
    let (head, raw_body) = resp
        .split_once("\r\n\r\n")
        .context("surfpool returned a malformed HTTP response")?;
    if !head.starts_with("HTTP/1.1 200") {
        bail!(
            "surfpool rpc call failed: {}",
    ...
    let parsed: Value = serde_json::from_str(&body)
        .with_context(|| format!("surfpool returned invalid JSON: {body}"))?;
    if let Some(err) = parsed.get("error") {
        let message = err
            .get("message")
            .and_then(Value::as_str)
            .unwrap_or("unknown error");
        // surfpool tucks the useful detail (e.g. "Cannot travel to past slot") in
        // `error.data`; include it when present.
        let detail = err.get("data").and_then(Value::as_str);
        match detail {
            Some(d) => bail!("surfpool rpc returned an error: {message}: {d}"),
            None => bail!("surfpool rpc returned an error: {message}"),
        }
    }
    Ok(body)
}
...
fn dechunk(body: &str) -> Result<String> {
    ...
    loop {
        let (size_line, after) = rest
            .split_once("\r\n")
            .context("malformed chunked response from surfpool")?;
        let size_hex = size_line.split(';').next().unwrap_or("").trim();
        let size = usize::from_str_radix(size_hex, 16)
            .with_context(|| format!("invalid chunk size '{size_hex}' from surfpool"))?;
        if size == 0 {
            break;
        }
        let chunk = after
            .get(..size)
            .context("truncated chunk in surfpool response")?;
        out.push_str(chunk);
...
/// `result` value. Solana JSON-RPC params are a positional array (e.g.
/// `json!([pubkey, {"encoding": "jsonParsed"}])`). Errors are surfaced by [`post`].
